...
#include<stdlib.h>
#include<iostream>
#include "heapForKruskal.cpp"
#include "listforKruskal.cpp"
...
void Graph::kruskal(MinHeap h, int m)
{
    LinkedListDoubly *aList = new LinkedListDoubly[nVertices];
    int *cluster=new int[nVertices];

    Graph mst;
    mst.setnVertices(nVertices);
    for(int i=0;i<nVertices;i++)
    {
        aList[i].insertFirst(i);
        cluster[i]=i;
    }
    int c=0,total=0;
    printf("Added edges:\n");
    while (c <nVertices-1)
    {
        ...
        {
            mst.addEdge(u,v,w);
            if(aList[cluster[u]].getLength()>=aList[cluster[v]].getLength())
            {
                ListNode *temp;
                temp=aList[v].getHead();
                while(temp!=0)
                {
                   cluster[temp->item]=cluster[u];
                   temp=temp->next;
                }
                aList[u].mergeList(aList[v]);
            }
            else
            {
                ListNode *temp;
                temp=aList[u].getHead();
                while(temp!=0)
                {
                   cluster[temp->item]=cluster[v];
                   temp=temp->next;
                }
                aList[v].mergeList(aList[u]);
            }
            total=total+w;
            printf("%d",u);
            printf(" , ");
